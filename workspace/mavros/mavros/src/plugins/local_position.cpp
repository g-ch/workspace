...
public:
	...
	void initialize(UAS &uas_)
	{
		...
		local_velocity = lp_nh.advertise<geometry_msgs::Vector3>("local_velocity", 20);//added by Clarence
	}
...
private:
	...
	ros::Publisher local_velocity; //added by Clarence
	...
	void handle_local_position_ned(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
        ...
        velocity.x = pos_ned.vy;
        velocity.y = pos_ned.vx;
        velocity.z = 0.0-pos_ned.vz;

		local_position.publish(pose);
		local_velocity.publish(velocity);
